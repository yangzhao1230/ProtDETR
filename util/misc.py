...
def load_sequences(which_dataset, nb_seq, with_label=False, with_roles=False):
    ...
    if which_dataset == "catalytic_site":
        dataframe = load_json_into_pandas_dataframe(
            "/blob/Project/Protein/catalytic_site_train.json"
        )
        # filter the sequence with length <= 1022
        # dataframe = dataframe[dataframe['sequence'].str.len() <= 1022]
        print(f"Number of sequences: {len(dataframe)}")
        list_catalytic_site = []
